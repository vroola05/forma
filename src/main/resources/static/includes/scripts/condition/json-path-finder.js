...
export class JsonPathFinder {
    ...
    static evalTokenized(path, input) {
        const results = [];
    ...
    static evaluateRecursive(tokens, tokenIndex, currentField, results) {
        ...
        if (token.startsWith('[')) {
            ...
            if ('*'.equals(content)) {
                ...
                const start = parts[0].length === 0 ? 0 : new Number(parts[0]);
                const end = parts[1].length === 0 ? children.length : new Number(parts[1]);

                for (let i = Math.max(0, start); i < Math.min(children.size(), end); i++) {
                    JsonPathFinder.evaluateRecursive(tokens, tokenIndex + 1, children[i], results);
                }
            } else {
                // Harde index (bijv. [2])
                const idx = new Number(content);
    ...
    static searchDeep(root, targetName, tokens, nextTokenIdx, results) {
