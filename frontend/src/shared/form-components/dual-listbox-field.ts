 ...
 * Dual Listbox
 */
export class DualListboxField extends InputNucleus<HTMLSelectElement> {
    values: OptionDto[];
    ...
    createElement() {
        ...
        this.inputElementDeselected = this.#createSelect(inputInnerWrapper,'deselected');
        inputInnerWrapper.appendChild(this.#createBtns());

        this.inputElement = this.#createSelect(inputInnerWrapper,'selected');
    ...
    #createSelect(container: HTMLElement, identifier: string) {
        ...
        element.id = `${this.getId()}-${identifier}`;
    ...
    #deselectOptions(selectedOptions: HTMLOptionElement[]) {
    ...
    #selectOptions(selectedOptions: HTMLOptionElement[]) {
    ...
    #orderList(element: HTMLSelectElement) {
    ...
    setValue(options: OptionDto[] | undefined, noCallback = false) {
        ...
        const optionsToSelect: HTMLOptionElement[] = [];
    ...
    #setSelectedValues(noCallback = false) {
        this.values = Array.from(this.inputElement.options).map(option => ({ 
            value: option.value,
            text: option.text,
            selected: true }));
